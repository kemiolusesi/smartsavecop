...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (funcError) {
      console.error('Error updating interests:', funcError);
      throw funcError;
    }
    ...
    if (plansError) {
      console.error('Error fetching matured plans:', plansError);
    } else if (activePlans && activePlans.length > 0) {
      console.log(`Found ${activePlans.length} plans ready for maturity`);
      ...
      for (const plan of activePlans) {
        ...
        if (updateError) {
          console.error(`Error marking plan ${plan.id} as matured:`, updateError);
        } else {
          console.log(`Plan ${plan.id} marked as matured`);
        }
      }
    }
    ...
    console.log('Daily accrual job completed:', result);
    ...
    console.error('Daily accrual job error:', error);
    return new Response(
      JSON.stringify({
