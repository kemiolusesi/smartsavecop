...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('savings_plans').select('*').eq('user_id', user.id);
    ...
    console.error('Savings plans API error:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Savings plans POST error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
